...
describe("Var", () => {
  ...
  test("test usage with default (limited)", () => {
    expect(textDecoration(varDefault("foo", "default")))
    ->toBe({"textDecoration": "var(--foo,default)"});
    expect(alignItems(varDefault("--bar", "default")))
    ->toBe({"alignItems": "var(--bar,default)"});
  });
});
...
describe("Color style", () =>
  test("test values", () => {
    ...
    expect(color(rgba(4, 5, 6, `num(0.3))))
    ->toBe({"color": "rgba(4, 5, 6, 0.3)"});
    expect(color(hsl(deg(7.), pct(8.), pct(9.))))
    ->toBe({"color": "hsl(7deg, 8%, 9%)"});
    expect(color(hsla(deg(10.), pct(11.), pct(12.), `num(0.5))))
    ->toBe({"color": "hsla(10deg, 11%, 12%, 0.5)"});
    expect(color(hsla(rad(4.7), pct(11.), pct(12.), pct(50.))))
    ->toBe({"color": "hsla(4.7rad, 11%, 12%, 50%)"});
...
describe("Label", () => {
  test("test value", () =>
    expect(label("a"))->toBe({"label": "a"})
  )
});

describe("Filter", () =>
  test("test values", () => {
    expect(filter([`opacity(10.), `invert(20.)]))
    ->toBe({"filter": "opacity(10%) invert(20%)"});
    expect(filter([`blur(`px(20)), `brightness(20.)]))
    ->toBe({"filter": "blur(20px) brightness(20%)"});
    expect(
      filter([
        `contrast(30.),
    ...
    expect(filter([`grayscale(10.), `hueRotate(`deg(180.))]))
    ->toBe({"filter": "grayscale(10%) hue-rotate(180deg)"});
    expect(filter([`saturate(10.), `sepia(100.)]))
    ->toBe({"filter": "saturate(10%) sepia(100%)"});
...
describe("Angle", () =>
  test("test values", () => {
    expect(transform(rotate(deg(1.))))
    ->toBe({"transform": "rotate(1deg)"});
    expect(transform(rotate(rad(6.28))))
    ->toBe({"transform": "rotate(6.28rad)"});
    expect(transform(rotate(grad(38.8))))
    ->toBe({"transform": "rotate(38.8grad)"});
    expect(transform(rotate(turn(0.25))))
    ->toBe({"transform": "rotate(0.25turn)"});
  })
);
...
describe("Backdrop filter", () =>
  test("test values", () => {
    ...
    expect(backdropFilter([`blur(`px(10)), `brightness(`percent(42.0))]))
    ->toBe({"backdropFilter": "blur(10px), brightness(42%)"});
    expect(
      backdropFilter([`contrast(`num(10)), `dropShadow(`percent(0.5))]),
    )
    ->toBe({"backdropFilter": "contrast(10), drop-shadow(0.5%)"});
    expect(
      backdropFilter([
    ...
    ->toBe({"backdropFilter": "grayscale(99.9%), hue-rotate(90deg)"});
    expect(backdropFilter([`invert(`num(30)), `opacity(`percent(10.0))]))
    ->toBe({"backdropFilter": "invert(30), opacity(10%)"});
    expect(backdropFilter([`saturate(`num(30)), `sepia(`percent(10.0))]))
    ->toBe({"backdropFilter": "saturate(30), sepia(10%)"});
  })
);
...
describe("Gradient background", () =>
  test("test values", () => {
    ...
    ->toBe({"background": "linear-gradient(45deg, #FF0000 0, #0000FF 100%)"});
    expect(
      background(
    ...
    ->toBe({"background": "radial-gradient(#FF0000 0, #0000FF 100%)"});
    expect(
      background(
...
describe("box-shadow", () => {
  test("should allow single or list definition", () => {
    expect(boxShadow(Shadow.box(green)))
    ->toBe({"boxShadow": "0 0 0 0 #008000"});
    expect(boxShadows([Shadow.box(yellow), Shadow.box(red)]))
    ->toBe({"boxShadow": "0 0 0 0 #FFFF00, 0 0 0 0 #FF0000"});
  });

  test("should use options when present", () => {
    expect(boxShadow(Shadow.box(~x=px(1), ~y=px(2), red)))
    ->toBe({"boxShadow": "1px 2px 0 0 #FF0000"});
    expect(boxShadow(Shadow.box(~inset=true, red)))
    ->toBe({"boxShadow": "0 0 0 0 #FF0000 inset"});
  });
  ...
  test("should allow special values", () => {
    ...
    expect(important(boxShadow(none)))
    ->toBe({"boxShadow": "none !important"});
  });
});

describe("text-shadow", () => {
  test("should allow single or list definition", () => {
    expect(textShadow(Shadow.text(green)))
    ->toBe({"textShadow": "0 0 0 #008000"});
    expect(textShadows([Shadow.text(yellow), Shadow.text(red)]))
    ->toBe({"textShadow": "0 0 0 #FFFF00, 0 0 0 #FF0000"});
  });

  test("should use options when present", () => {
    expect(textShadow(Shadow.text(~x=px(1), ~y=px(2), red)))
    ->toBe({"textShadow": "1px 2px 0 #FF0000"});
    expect(textShadow(Shadow.text(~blur=vh(1.), red)))
    ->toBe({"textShadow": "0 0 1vh #FF0000"});
  });
  ...
  test("should allow special values", () => {
    ...
    expect(important(textShadow(none)))
    ->toBe({"textShadow": "none !important"});
  });
});
...
describe("transitions", () => {
  test("should allow single or list definition", () => {
    ...
    ->toBe({"transition": "0ms ease 0ms transform"});
    expect(
      transitions([
    ...
    ->toBe({"transition": "0ms ease 0ms height, 0ms ease 0ms top"});
  });

  test("should use options when present", () =>
    expect(transition(~duration=3, ~delay=4, ~timingFunction=easeOut, "top"))
    ->toBe({"transition": "3ms ease-out 4ms top"})
  );
});
...
describe("animation", () => {
  test("should allow single or list definition", () => {
    expect(animation(toAnimationName("a")))
    ->toBe({"animation": "a 0ms ease 0ms 1 normal none running"});
    expect(
      animations([
...
describe("gridTemplateAreas", () => {
  test("takes acceptable types & cascades", () => {
    ...
    expect(gridTemplateAreas(`areas(["a"])))
    ->toBe({"gridTemplateAreas": "'a'"});
    expect(gridTemplateAreas(`inherit_))
    ->toBe({"gridTemplateAreas": "inherit"});
    expect(gridTemplateAreas(`initial))
    ->toBe({"gridTemplateAreas": "initial"});
    expect(gridTemplateAreas(`unset))->toBe({"gridTemplateAreas": "unset"});
  });

  test("sucessfully combines list", () => {
    expect(gridTemplateAreas(`areas(["a a a", "b b b"])))
    ->toBe({"gridTemplateAreas": "'a a a' 'b b b'"})
  });
});
...
describe("GridArea", () => {
  ...
  test("multi-arg functions add in slashes", () => {
    ...
    expect(gridArea3(`ident("a"), `numIdent((1, "a")), `auto))
    ->toBe({"gridArea": "a / 1 a / auto"});
    expect(
      gridArea4(`num(5), `span(`num(16)), `span(`ident("b")), `auto),
    )
    ->toBe({"gridArea": "5 / span 16 / span b / auto"});
  });
});

describe("gridTemplateCoumns", () => {
  test("concatenates list", () =>
    expect(gridTemplateColumns([`fr(1.), `px(100), `auto]))
    ->toBe({"gridTemplateColumns": "1fr 100px auto"})
  );

  test("unfolds repeats", () => {
    expect(gridTemplateColumns([`repeat((`num(4), `fr(1.)))]))
    ->toBe({"gridTemplateColumns": "repeat(4, 1fr)"});
    expect(gridTemplateColumns([`repeat((`num(4), `auto))]))
    ->toBe({"gridTemplateColumns": "repeat(4, auto)"});
    expect(gridTemplateColumns([`repeat((`num(4), `minContent))]))
    ->toBe({"gridTemplateColumns": "repeat(4, min-content)"});
    expect(gridTemplateColumns([`repeat((`num(4), `maxContent))]))
    ->toBe({"gridTemplateColumns": "repeat(4, max-content)"});
    expect(
      gridTemplateColumns([
        `repeat((`num(4), `minmax((`px(100), `fr(1.))))),
      ]),
    )
    ->toBe({"gridTemplateColumns": "repeat(4, minmax(100px,1fr))"});
  });
});
...
describe("backgroundPosition", () => {
  test("test single values", () => {
    ...
    expect(backgroundPosition(`right))
    ->toBe({"backgroundPosition": "right"});
    expect(backgroundPosition(`top))->toBe({"backgroundPosition": "top"});
    expect(backgroundPosition(`bottom))
    ->toBe({"backgroundPosition": "bottom"});
    expect(backgroundPosition(center))
    ->toBe({"backgroundPosition": "center"});
    expect(backgroundPosition(pct(50.)))
    ->toBe({"backgroundPosition": "50%"});
    expect(backgroundPosition(initial))
    ->toBe({"backgroundPosition": "initial"});
    expect(backgroundPosition(inherit_))
    ->toBe({"backgroundPosition": "inherit"});
    expect(backgroundPosition(unset))
    ->toBe({"backgroundPosition": "unset"});
  });

  test("test two values", () => {
    expect(backgroundPosition(`hv((`left, center))))
    ->toBe({"backgroundPosition": "left center"});
    expect(backgroundPosition(`hv((`right, pct(50.)))))
    ->toBe({"backgroundPosition": "right 50%"});
    expect(backgroundPosition(`hv((pct(50.), `top))))
    ->toBe({"backgroundPosition": "50% top"});
    expect(backgroundPosition(`hv((pct(50.), pct(50.)))))
    ->toBe({"backgroundPosition": "50% 50%"});
  });

  test("test multiple positions", () =>
    expect(backgroundPositions([`hv((px(0), px(0))), center]))
    ->toBe({"backgroundPosition": "0px 0px, center"})
  );
  ...
  test("test edge offsets values", () =>
    ...
    ->toBe({"backgroundPosition": "right 50px top 10px"})
  );
});

describe("backgroundRepeat", () => {
  test("test single values", () => {
    expect(backgroundRepeat(repeatX))
    ->toBe({"backgroundRepeat": "repeat-x"});
    expect(backgroundRepeat(repeatY))
    ->toBe({"backgroundRepeat": "repeat-y"});
    ...
    expect(backgroundRepeat(noRepeat))
    ->toBe({"backgroundRepeat": "no-repeat"});
    expect(backgroundRepeat(inherit_))
    ->toBe({"backgroundRepeat": "inherit"});
  });

  test("test two values", () => {
    expect(backgroundRepeat(`hv((repeat, space))))
    ->toBe({"backgroundRepeat": "repeat space"});
    expect(backgroundRepeat(`hv((repeat, repeat))))
    ->toBe({"backgroundRepeat": "repeat repeat"});
    expect(backgroundRepeat(`hv((round, space))))
    ->toBe({"backgroundRepeat": "round space"});
    expect(backgroundRepeat(`hv((noRepeat, round))))
    ->toBe({"backgroundRepeat": "no-repeat round"});
  });
});
...
describe("backgroundImage", () =>
  test("test values", () => {
    ...
    ->toBe({"backgroundImage": "linear-gradient(5deg, #FF0000 10%)"});
    expect(
      backgroundImage(
    ...
    expect(backgroundImage(radialGradient([|(pct(30.), yellow)|])))
    ->toBe({"backgroundImage": "radial-gradient(#FFFF00 30%)"});
    expect(
      backgroundImage(repeatingRadialGradient([|(pct(30.), yellow)|])),
    )
    ->toBe({"backgroundImage": "repeating-radial-gradient(#FFFF00 30%)"});
  })
);
...
describe("background shorhand", () =>
  test("test values", () => {
    ...
    expect(background(linearGradient(deg(5.), [|(pct(10.), red)|])))
    ->toBe({"background": "linear-gradient(5deg, #FF0000 10%)"});
...
describe("gap", () => {
  ...
  test("test values two props", () => {
    expect(gap2(~rowGap=normal, ~columnGap=normal))
    ->toBe({"gap": "normal normal"});
    expect(gap2(~rowGap=px(3), ~columnGap=px(3)))
    ->toBe({"gap": "3px 3px"});
    expect(gap2(~rowGap=em(2.5), ~columnGap=em(2.5)))
    ->toBe({"gap": "2.5em 2.5em"});
    expect(gap2(~rowGap=pct(3.), ~columnGap=pct(3.)))
    ->toBe({"gap": "3% 3%"});
    expect(gap2(~rowGap=inherit_, ~columnGap=inherit_))
    ->toBe({"gap": "inherit inherit"});
    expect(gap2(~rowGap=initial, ~columnGap=initial))
    ->toBe({"gap": "initial initial"});
    expect(gap2(~rowGap=unset, ~columnGap=unset))
    ->toBe({"gap": "unset unset"});
  });
});
...
describe("counter", () => {
  test("test reset", () => {
    ...
    expect(counterReset(`reset(("foo", 2))))
    ->toBe({"counterReset": "foo 2"});
  ...
  test("test increment", () => {
    ...
    expect(counterIncrement(`increment(("foo", 2))))
    ->toBe({"counterIncrement": "foo 2"});
    expect(counterIncrement(var("bar")))
    ->toBe({"counterIncrement": "var(--bar)"});
  });
});
